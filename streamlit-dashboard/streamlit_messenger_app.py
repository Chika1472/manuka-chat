...
BASE_MODEL_DIR_TEXT = "/mount/src/manuka-chat/model/manuka-model-0611"
BASE_MODEL_DIR = Path(BASE_MODEL_DIR_TEXT)
LOCAL_MODEL_DIR = APP_DIR.parent / "model" / "manuka-model-0611"
DEFAULT_MODEL_DIR_TEXT = BASE_MODEL_DIR_TEXT
NOTEBOOK_GENERATION_DEFAULTS = {
    "max_new_tokens": 192,
    "min_new_tokens": 24,
    "temperature": 0.72,
    "top_p": 0.88,
...
def resolve_model_dir(model_dir_text: str) -> Path:
    requested = Path(model_dir_text).expanduser()
    normalized_text = model_dir_text.replace("\\", "/")
    if normalized_text == BASE_MODEL_DIR_TEXT and not requested.exists() and LOCAL_MODEL_DIR.exists():
...
def model_file_state(model_dir: Path, cfg: dict[str, Any] | None) -> tuple[list[dict[str, str]], bool]:
    ...
    tokenizer_name = (cfg or {}).get("tokenizer_model", "spm32k.model")
        ...
        st.markdown(
            ...
            <section class="chat-frame">
                <div class="empty-card">
                    <div class="empty-kicker">MANUKA CHECKPOINT</div>
                    <h1 class="empty-title">Manuka Chat</h1>
                </div>
            </section>
            """,
            unsafe_allow_html=True,
...
with st.container(key="settings_popover"):
    ...
    with right:
        with st.popover("Settings", use_container_width=True):
            ...
            model_dir_text = st.text_input("Model directory", value=DEFAULT_MODEL_DIR_TEXT)
            model_dir = resolve_model_dir(model_dir_text)
            ...
            default_max_new = min(
                int(gen_cfg.get("max_new_tokens", NOTEBOOK_GENERATION_DEFAULTS["max_new_tokens"])),
                max_new_limit,
            )
            max_new_tokens = st.slider("Max new tokens", 1, max_new_limit, default_max_new)
            default_min_new = min(int(gen_cfg.get("min_new_tokens", NOTEBOOK_GENERATION_DEFAULTS["min_new_tokens"])), max_new_tokens)
            min_new_tokens = st.slider("Min new tokens", 0, min(96, max_new_tokens), default_min_new)
            temperature = st.slider(
                "Temperature",
                0.1,
                1.5,
                float(gen_cfg.get("temperature", NOTEBOOK_GENERATION_DEFAULTS["temperature"])),
                0.01,
            )
            top_p = st.slider("Top p", 0.05, 1.0, float(gen_cfg.get("top_p", NOTEBOOK_GENERATION_DEFAULTS["top_p"])), 0.01)
            top_k = st.slider("Top k", 0, 200, int(gen_cfg.get("top_k", NOTEBOOK_GENERATION_DEFAULTS["top_k"])))
            repetition_penalty = st.slider(
                "Repetition penalty",
                1.0,
                1.5,
                float(gen_cfg.get("repetition_penalty", NOTEBOOK_GENERATION_DEFAULTS["repetition_penalty"])),
                0.01,
            )
            no_repeat_ngram_size = st.slider(
                ...
                0,
                8,
                int(gen_cfg.get("no_repeat_ngram_size", NOTEBOOK_GENERATION_DEFAULTS["no_repeat_ngram_size"])),
            )
            eos_requires_sentence_end = st.checkbox(
                "EOS requires sentence end",
                value=bool(gen_cfg.get("eos_requires_sentence_end", NOTEBOOK_GENERATION_DEFAULTS["eos_requires_sentence_end"])),
            )
        ...
        try:
            ...
            with st.spinner("Generating response"):
                ...
                result = generator.generate_reply(
                    ...
                    eos_requires_sentence_end=eos_requires_sentence_end,
