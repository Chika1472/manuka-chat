...
class ManukaGenerator:
    ...
    def _load_state_dict(self, path: Path) -> dict[str, torch.Tensor]:
        ...
        try:
            ...
            trusted_checkpoints = {
                (Path("/mount/src/manuka-chat/model/manuka-model-0611") / "model.pt").resolve(),
                (Path(__file__).resolve().parent.parent / "model" / "manuka-model-0611" / "model.pt").resolve(),
            }
            if path.resolve() not in trusted_checkpoints:
                raise
    ...
    def can_stop_on_eos(
        ...
        if not eos_requires_sentence_end:
            return True
        return bool(self.sentence_end_regex.search(self.decode_text(generated).strip()))
