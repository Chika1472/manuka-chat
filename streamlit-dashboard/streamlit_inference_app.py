...
st.markdown(
    ...
    div[data-testid="stPopover"] button {
        ...
        color: var(--ink);
        box-shadow: 0 12px 34px rgba(20, 20, 19, 0.07);
        font-size: 24px;
        font-weight: 300;
        padding: 0;
    }
    ...
    .stTextArea textarea:focus {
        ...
        box-shadow:
            ...
            0 12px 34px rgba(20, 20, 19, 0.07);
    }
    .stTextInput input,
    .stNumberInput input,
    ...
    [data-testid="baseButton-primary"] {
        ...
        color: #ffffff;
        box-shadow: 0 12px 34px rgba(204, 120, 92, 0.22);
        font-size: 14px;
        font-weight: 600;
    }
...
def render_status_list(checks: list[dict[str, str]]) -> None:
    ...
    for check in checks:
        ...
        rows.append(
            f"""
            <div class="status-row status-{state}">
...
st.markdown(
    f"""
    <div class="top-mark" aria-hidden="true"><span></span></div>
...
settings_col, prompt_col, run_col = st.columns([0.52, 6.5, 0.92], gap="small")
with settings_col:
...
with prompt_col:
    prompt = st.text_area("Prompt", value="", placeholder="Ask anything", height=58, label_visibility="collapsed")

with run_col:
    generate_clicked = st.button("Run", type="primary", use_container_width=True)
